...
import { createAssetMetadata, deleteAsset, toggleAssetFinal } from "@/app/actions/assets";
import {
...
export function AssetGallery({ initialAssets, pieceId }: Props) {
    ...
    async (files: FileList | File[]) => {
      ...
      for (let i = 0; i < fileArray.length; i++) {
        ...
        try {
          ...
          // Insert metadata using Server Action
          const res = await createAssetMetadata({
  ...
  const handleToggleFinal = async (asset: Asset) => {
    ...
    await toggleAssetFinal(asset.id, newVal);
    setAssets((prev) =>
    ...
    >
      ...
      <div className="aspect-video bg-[#F8F6F6] flex items-center justify-center overflow-hidden relative">
        {asset.file_type === "image" ? (
          ...
          <img
            src={getProxyUrl(asset.file_url)}
        ...
        {showControls && (
          <div className="absolute inset-0 bg-[rgba(28,28,28,0.45)] flex items-center justify-center gap-2">
            ...
            <a
              href={getProxyUrl(asset.file_url)}
              target="_blank"
