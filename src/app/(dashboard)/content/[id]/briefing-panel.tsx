...
import { upsertBriefing } from "@/app/actions/briefings";
...
export function BriefingPanel({ initialBriefing, pieceId }: Props) {
  ...
  const handleSave = async () => {
    ...
    const result = await upsertBriefing({
      id: briefing?.id,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        {saveError && (
          <p className="text-[10px] font-bold uppercase tracking-widest text-[#FF3B30]">
            Error: {saveError}
          </p>
        )}
        <button
