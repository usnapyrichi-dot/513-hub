...
export default function SettingsPage() {
  const getLocal = (key: string, def: string) =>
    typeof window !== "undefined" ? (localStorage.getItem(key) ?? def) : def;

  const [agencyName,    setAgencyName]    = useState(() => getLocal("513hub_agency_name", "513 HUB"));
  const [agencyTagline, setAgencyTagline] = useState(() => getLocal("513hub_agency_tagline", "Editorial Studio"));
  const [notifStatus,   setNotifStatus]   = useState(() => getLocal("513hub_notif_status", "true") !== "false");
  const [notifReview,   setNotifReview]   = useState(() => getLocal("513hub_notif_review", "true") !== "false");
  ...
  const handleSave = async () => {
    ...
    localStorage.setItem("513hub_agency_name",    agencyName);
    localStorage.setItem("513hub_agency_tagline", agencyTagline);
    localStorage.setItem("513hub_notif_status",   String(notifStatus));
    localStorage.setItem("513hub_notif_review",   String(notifReview));
    setSaved(true);
    setSaving(false);
