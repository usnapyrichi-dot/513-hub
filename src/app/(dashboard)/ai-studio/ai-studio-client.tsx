...
// ─── localStorage key for Gem instructions ────────────────────────────────────
const gemKey = (clientId: string) => `513hub_gem_${clientId}`;
...
export function AIStudioClient({ clients, initialPrompts }: Props) {
  ...
  // ── Load Gem from localStorage when client changes ────────────────────────
  useEffect(() => {
    if (!clientId) return;
    const stored = localStorage.getItem(gemKey(clientId)) ?? "";
    setGemInstruction(stored);
    setGemDraft(stored);
  }, [clientId]);

  // ── Save Gem to localStorage ──────────────────────────────────────────────
  const handleSaveGem = useCallback(() => {
    localStorage.setItem(gemKey(clientId), gemDraft);
    setGemInstruction(gemDraft);
    setGemSaved(true);
    setTimeout(() => { setGemSaved(false); setGemOpen(false); }, 1200);
  }, [clientId, gemDraft]);

  const handleClearGem = useCallback(() => {
    localStorage.removeItem(gemKey(clientId));
    setGemInstruction("");
    setGemDraft("");
  }, [clientId]);
