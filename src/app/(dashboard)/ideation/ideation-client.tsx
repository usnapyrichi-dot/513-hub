...
export function IdeationClient({ clients }: { clients: any[] }) {
  ...
  useEffect(() => {
    const gem = localStorage.getItem(`513hub_gem_${selectedClientId}`);
    setHasGem(!!gem?.trim());
  }, [selectedClientId]);
  ...
  const handleSaveDraft = async () => {
    ...
    if (saveMode === "new") {
      ...
      if (error) {
        ...
        setHasSaved(true);
        setTimeout(() => { setHasSaved(false); router.push(`/content/${data.id}`); }, 1200);
      }
    } else {
  ...
  const handleGenerate = async () => {
    ...
    // Load Gem instruction for this client (same key as AI Studio)
    const gem = typeof window !== "undefined"
      ? (localStorage.getItem(`513hub_gem_${selectedClientId}`) ?? undefined)
      : undefined;

    const result = await generateContentIdeas(selectedClientId, prompt, gem || undefined);
