...
import { ArrowLeft, CarFront, FileText, Image as ImageIcon } from "lucide-react";
          ...
          <div className="flex-1 space-y-6">
            <div className="flex justify-between items-end mb-8 border-b border-[#E5E5E5] pb-4">
               <div>
                 <h3 className="font-headline font-bold text-2xl uppercase tracking-widest text-[#1C1C1C] flex items-center gap-3">
                   <CarFront className="w-6 h-6 text-[var(--color-accent-red)]" />
                   Catálogo de Vehículos
                 </h3>
                 <p className="text-[#949494] text-sm font-medium mt-2">Modelos asignados a campañas y contenidos</p>
               </div>
            ...
            {(!carModels || carModels.length === 0) ? (
               <div className="w-full h-[300px] border-2 border-dashed border-[#E5E5E5] rounded-sm flex flex-col items-center justify-center text-[var(--color-text-muted)] bg-white/50">
                  <CarFront size={32} className="text-[#D1D1D1] mb-4" />
                  <p className="font-headline font-bold text-sm uppercase tracking-widest text-[#1C1C1C]">Vacío</p>
                  <p className="text-xs mt-2 text-[#949494]">No hay modelos registrados para esta marca aún.</p>
               </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {carModels.map((car) => (
                  <div key={car.id} className="bg-white border border-[#E5E5E5] rounded-sm p-6 shadow-sm hover:border-[#1C1C1C] transition-colors group">
                    <div className="flex justify-between items-start mb-4">
                       <h4 className="text-xl font-headline font-bold text-[#1C1C1C] uppercase group-hover:text-[var(--color-accent-red)] transition-colors">{car.name}</h4>
                       <span className="text-[10px] px-2 py-1 bg-[#F8F6F6] text-[#949494] uppercase tracking-widest font-bold rounded-sm border border-[#E5E5E5]">{car.year}</span>
                    </div>
                    
                    <div className="space-y-2 mb-6">
                      <div className="grid grid-cols-[80px_1fr] text-xs">
                        <span className="text-[#949494] font-bold uppercase tracking-widest text-[10px]">Segmento</span>
                        <span className="text-[#1C1C1C] font-medium">{car.category || 'N/A'}</span>
                      </div>
                    </div>

                    <div className="flex gap-2">
                      <Button variant="outline" className="flex-1 bg-[#FAFAFA] text-[10px] h-8 border-[#E5E5E5] uppercase tracking-widest font-bold text-[#949494] hover:text-[#1C1C1C]">
                        Ver Ficha
                      </Button>
                    </div>
                  </div>
