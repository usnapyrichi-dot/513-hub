...
export async function generateContentIdeas(clientId: string, prompt: string, gemInstruction?: string) {
  try {
    let finalGem = gemInstruction;
    if (finalGem === undefined) {
      const gems = await listGems(clientId);
      if (gems && gems.length > 0) finalGem = gems[0].system_instruction;
    }
...
`;
    const resultText = await callGemini(fullPrompt, 0.8, 8192, finalGem);
...
`;
    const resultText = await callGemini(fullPrompt, 0.75, 8192, finalGem);
...
export async function saveAiPrompt(data: {
  ...
  const { error, data: saved } = await supabase
    .from("ai_prompts")
    .insert({ workspace_id, ...data, is_favorite: false })
    .select()
    .single();
